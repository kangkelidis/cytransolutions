...
export default function ResizableDiv({ rideInfo }) {
  ...
  const [height, setHeight] = React.useState(rideInfo.duration ? rideInfo.duration : 60);
  ...
  return (
    <div className=""
     ...
     >
      <div className=" text-sm absolute bg-slate-900 rounded-t-lg w-[100px] pl-2 "
      ...
      >
        <div
          ...
          overflow-scroll no-scrollbar select-none rounded-b-md ${editable ? "cursor-move" : "cursor-help"}
           border-red-600 border-dashed ${editable && "border-[3px]"}`}
        ...
        >
          ...
          <div className="absolute z-50 left-[100px] bg-black rounded-md px-2 flex flex-col">
            <span className="  ">{rideInfo && rideInfo.driver}</span>
              <span>{String(Math.floor(height / 60)) +"h"+height%60+"m"}</span>
