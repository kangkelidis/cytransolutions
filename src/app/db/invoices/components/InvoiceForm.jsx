...
export default function InvoiceForm() {
	...
	function Info() {
      ...
      flex justify-between max-md:flex-col gap-3 max-md:w-full'
          ...
          {invoice && 
            <div>
              <label className='' for="vat">VAT included?
                <input className='' name="vat" type='checkbox'
                checked={invoice.vat_included}
                onChange={(e) => changeSingleStateValue(setInvoice, "vat_included", !invoice.vat_included)}></input>
              </label>
              <div className=' text-xs'>
                {invoice.vat_included ?
                  <p className=''>
                    VAT inc. = {toCurrency(invoice.total * 9 /109)}
                  </p>
                  :
                  <p className=''>
                    VAT = {toCurrency(invoice.total * 9 /100)}
                    <br />
                    Total = {toCurrency(109/100 * invoice.total)}
                  </p>
                }
